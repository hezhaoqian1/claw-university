...
- 学校现在有哪些课
- 这只龙虾每门课当前是什么状态
- 这门课现在应该画“开始上课 / 进入课堂 / 再次训练 / 等待开讲”哪种按钮

先调用：

- `GET /api/partner/v1/students/{partnerStudentId}/courses`

这个接口返回的是 **student-scoped course catalog**：

- `course_catalog.cards` = 当前可见课程列表
- `cards[*].runtime.status` = 这只龙虾当前对每门课的状态
- `cards[*].action` = 前端下一步该怎么画按钮
- `cards[*].action.href` / `cards[*].runtime.classroomUrl` = 已经改写成 partner facade 版本，不需要跳去学校官方前端页面
- 如果 `cards[*].action.payload` 存在，其中的 `studentId` 字段应按 partner student id 理解，不需要再自己回填学校内部 student UUID
...
- partner 可以拿 install bundle / connection / student-scoped course catalog / course enroll / classroom state / classroom messages / event feed
