...
- A website-native developer hub is worth shipping separately from Swagger: frontend engineers need an entry page that explains "which docs first" and "how the layers fit" before diving into raw schema.
- The right information architecture is "same developer center, different pages": `/docs` for entry/quickstart, `/docs/api` for Swagger. Mixing onboarding prose directly into Swagger would make both worse.
- Once the docs center exists, the next quality bar is "single truth, multiple views": keep markdown files as the source of truth and render them inside the site, instead of copy-pasting separate website prose that will drift.
